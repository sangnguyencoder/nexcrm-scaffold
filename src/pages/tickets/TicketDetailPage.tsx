...
export function TicketDetailPage() {
  ...
  const { data: ticket, isLoading } = useTicketDetailQuery(id);
  ...
  const saveTitle = () => {
    if (titleDraft.trim() && titleDraft.trim() !== ticket.title) {
      updateTicket.mutate({ title: titleDraft.trim() });
          ...
          <CardContent className="space-y-4">
            ...
            <Field label="Ưu tiên">
              <Select
                ...
                onChange={(event) => {
                  updateTicket.mutate({ priority: event.target.value as typeof ticket.priority });
            ...
            <Field label="Phụ trách">
              ...
              <Select
                ...
                onChange={(event) => {
                  updateTicket.mutate({ assigned_to: event.target.value });
                  void queryClient.invalidateQueries({ queryKey: queryKeys.comments });
